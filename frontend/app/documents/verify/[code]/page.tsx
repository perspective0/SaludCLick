...
export default function PublicDocumentVerificationPage() {
  ...
  const verifyDocument = async () => {
    try {
      setLoading(true);
      setError('');
      const response = await fetch(`${API_URL}/lab-orders/verify/${encodeURIComponent(code)}`, {
        cache: 'no-store',
      });
      const payload = await response.json().catch(() => null);

      if (!response.ok || !payload?.success) {
        throw new Error(payload?.message || 'No se pudo verificar el documento.');
      }

      setResult(payload.data);
    } catch (err: any) {
      setResult(null);
      setError(err?.message || 'No se pudo verificar el documento.');
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="space-y-6">
              <div className={`rounded-2xl border p-5 ${valid ? 'border-emerald-200 bg-emerald-50' : 'border-rose-200 bg-rose-50'}`}>
                <div className="flex items-start gap-3">
                  <FileCheck2 className={`mt-0.5 h-5 w-5 ${valid ? 'text-emerald-700' : 'text-rose-700'}`} />
                  <div>
                    <p className={`font-black ${valid ? 'text-emerald-800' : 'text-rose-800'}`}>
                      {valid ? 'Documento autentico emitido por SaludClick.' : 'El documento no esta activo.'}
                    </p>
                    <p className="mt-1 text-sm text-gray-600">Confirma que el codigo impreso coincida exactamente con el mostrado aqui.</p>
                  </div>
                </div>
              </div>

              <div className="grid gap-4 md:grid-cols-2">
                <Info label="Documento" value={formatDocumentLabel(result)} />
                ...
                <Info label="Emitido" value={formatDate(result.issuedAt)} />
                <Info label="Codigo" value={result.validationCode || code} />
              </div>
            </div>
          )}
        </section>
...
function formatDocumentLabel(result: any) {
  const title = result?.documentName || result?.documentTitle || 'Documento medico';
  const category = result?.documentCategory;
  if (!category || title.toLowerCase().includes(String(category).toLowerCase())) return title;
